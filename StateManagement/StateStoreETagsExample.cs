...
namespace Samples.Client
{
    public class StateStoreETagsExample : Example
    {
        ...
        public override async Task RunAsync(CancellationToken cancellationToken)
        {
            ...
            await daprClient.SaveStateAsync<Widget>(storeName, stateKeyName,  new Widget() { Size = "small", Color = "yellow", }, cancellationToken: cancellationToken);
            Console.WriteLine($"Saved state which created a new entry with an initial etag");

            // Read the state and etag
            var (original, originalETag) = await daprClient.GetStateAndETagAsync<Widget>(storeName, stateKeyName, cancellationToken: cancellationToken);
            ...
            var isSaveStateSuccess = await daprClient.TrySaveStateAsync<Widget>(storeName, stateKeyName, original, originalETag, cancellationToken: cancellationToken);
            Console.WriteLine($"Saved state with old etag: {originalETag} successfully? {isSaveStateSuccess}");

            // Read the updated state and etag
            var (updated, updatedETag) = await daprClient.GetStateAndETagAsync<Widget>(storeName, stateKeyName, cancellationToken: cancellationToken);
            ...
            updated.Color = "green";
            isSaveStateSuccess = await daprClient.TrySaveStateAsync<Widget>(storeName, stateKeyName, updated, updatedETag, cancellationToken: cancellationToken);
            ...
            var isDeleteStateSuccess = await daprClient.TryDeleteStateAsync(storeName, stateKeyName, originalETag, cancellationToken: cancellationToken);
            Console.WriteLine($"Deleted state with old etag: {originalETag} successfully? {isDeleteStateSuccess}");

            // Read the updated state and etag
            (updated, updatedETag) = await daprClient.GetStateAndETagAsync<Widget>(storeName, stateKeyName, cancellationToken: cancellationToken);
            Console.WriteLine($"Retrieved state: {updated.Size}  {updated.Color} with etag: {updatedETag}");

            // Delete with updated etag. This will succeed
            isDeleteStateSuccess = await daprClient.TryDeleteStateAsync(storeName, stateKeyName, updatedETag, cancellationToken: cancellationToken);
