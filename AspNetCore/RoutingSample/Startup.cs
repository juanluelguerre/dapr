    ...
    public class Startup
    {
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env, JsonSerializerOptions serializerOptions)
        {
            ...
            app.UseEndpoints(endpoints =>
            {
                ...
                endpoints.MapPost("forecast", DoForecast).WithTopic(PubsubName, "forecast");
            });
            ...
            async Task DoForecast(HttpContext context)
            {
                Console.WriteLine("Enter Forecast");

                var client = context.RequestServices.GetRequiredService<DaprClient>();

                var forecast = await JsonSerializer.DeserializeAsync<WeatherForecast>(context.Request.Body, serializerOptions);
                Console.WriteLine("Id is {0}, Amount is {1}", forecast.TemperatureF, forecast.Summary);
                ...
                context.Response.ContentType = "application/json";
                await JsonSerializer.SerializeAsync(context.Response.Body, forecast, serializerOptions);
            }
        }
    }
}
